...
function CreateGameBox(callback){ //Callback function will be passed with array of game box.
  ...
  console.log("New");
  FindClustersHandler(gameBoxArray,RemoveCluster,null);
...
function FindClustersHandler(array,callback,interval){
	for(var i = 1;i<9;i++){
  	var j = 1;
    var startingValue = parseInt(""+i+j);
    var endValue = startingValue+7;
   	FindClusters(array,startingValue,endValue,"H",callback,null);
    startingValue = parseInt(""+1+i);
  	endValue = startingValue + 70;
    FindClusters(array,startingValue,endValue,"V",callback,interval);
  }
...
function FindClusters(array,startingValue,endValue,direction,callback,interval){
...
function PopElements(clusterArray,array,socket){
  for(var i = 0;i<clusterArray.length;i++){
    ...
    for(var l = (elementsToReposition.length)-1; l>=0;l--){
      ...
      array[home].element = 6;
    }
    setTimeout(function(){socket.emit("Redraw",{array : array});},70);
    console.log("Elements to pop");
    console.log(elementsToPop);
    console.log("Cluster array");
