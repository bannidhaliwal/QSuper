      ...
      GameBox.CreateGameBox(function(arrayArg){
        ...
        socket.on("ClickedOnCanvas",function (data) {
          if(clicked) {
            ...
            validSwap(clickedElement,destinationId,arrayArg,function(possibleMovements){//Pass the data to the callback function
              CheckIfMoveIsValid(possibleMovements,destinationId,data);
            });
...
function CheckIfMoveIsValid(possibleMovements,destinationId,data){
  var destinationValid = false;
  for(var i = 0;i<possibleMovements.length;i++){//if that element is in the list of valid movements..
    if(destinationId === possibleMovements[i]){
      destinationValid = true;
    }
  }
  if(destinationValid){
    console.log("This movement can be made but we dont have that configuration yet.");
  }
  else{
    console.log("This movement cannot be made as the movement is illegal");
  }
}
...
function AssignPositions(callback,currentPosition,positionCategory,gameArray,destinationId){
  ...
  DetectCombination(currentPosition,destinationId,gameArray);
  callback(validMovements);
...
function DetectCombination(currentPosition,destinationId,gameArray){
  var movementPossible = false;
  console.log("current position is "+currentPosition + "and destination is "+destinationId);
  ...
  if(movementPossible){console.log("Movement can be made");}else{console.log("movement Illegal");}
}
