    ...
    var potentialSwappingCandidate = null;
      GameBox.CreateGameBox(function(arrayArg){
        ...
        socket.on("ClickedOnCanvas",function (data) {
          if(clicked) {
            validSwap(clickedElement,function(possibleMovements){
              var destinationElement = parseInt(WhichElementWasClicked(data.X,data.Y));
              var destinationValid = false;
              for(var i = 0;i<possibleMovements.length;i++){
                if(destinationElement === possibleMovements[i]){
...
function validSwap(movingElement,callback){
  DetermineThePositionOfClickedElement(movingElement,callback);
}//End validSwap
...
function DetermineThePositionOfClickedElement(id,callback){
  ...
  AssignPositions(callback,id,possibleMovement);
...
//function.
function AssignPositions(callback,currentPosition,positionCategory){
  var locations = {LE:"",RE:"",BRE:"",BLE:"",LC:"",RC:"",UR:"",BR:"",AL:""};
  currentPosition = parseInt(currentPosition);//we need to parse as we need the string form for our other funcitons.s
  ...
  callback(locations[positionCategory]);
