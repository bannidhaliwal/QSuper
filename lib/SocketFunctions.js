      ...
      GameBox.CreateGameBox(function(arrayArg){
        ...
        socket.on("ClickedOnCanvas",function (data) {
          if(clicked) {
            ...
            //Check for the clusters after player move the element..
              setTimeout(function(){ClusterFunction.FindClustersHandler(arrayArg,function(modifiedGameArray,array){
...
function DetectCombination(currentPosition,destinationId,gameArray,callback){
  ...
  if(movementPossible){
    ...
    //In the callback function we can pass the argument of the destination and
    //current position after swapping and than pop the elements in the callback
    //function
    callback(gameArray);
  }
}
