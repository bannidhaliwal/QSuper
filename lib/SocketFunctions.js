...
//This function is the main point to initialize the game box.
//In this function we emit the socket gameInitialized event for the
//client side to draw the game box.
function SocketEvents(ioObject,GameBox){
  ioObject.sockets.on('connection', function (socket) {
    var clicked = false;
    var score = 0;//This varable will hold the score for the connected client..
        ...
        socket.on("ClickedOnCanvas",function (data) {
          ...
          if(clicked) {
            ...
            validSwap(clickedElement,destinationId,arrayArg,function(modifiedGameArray){//Pass the data to the callback function
              ...
              CheckIfMoveIsValid(modifiedGameArray,socket,{dest : destinationId,home : clickedElement,destElement : arrayArg[parseInt(destinationId)].element,homeElement : arrayArg[parseInt(clickedElement)].element});
            });
              //Check for the clusters repeatedly until we dont have any cluster left.
              //We also giving some time swap animation.
              var clusterInterval = setInterval(function(){
              ClusterFunction.FindClustersHandler(arrayArg,function(modifiedGameArray,array){
                //We use this function inside the callback to pass data to this function
                ClusterFunction.PopElements(modifiedGameArray,arrayArg,socket);
              },
              clusterInterval//Interval to clear
            );}
            ,130);//End of clusterInterval.
            socket.emit("Dehighlight",{id : clickedElement,element : arrayArg[parseInt(clickedElement)].element})
            clicked = false;
            setTimeout(function(){clearInterval(clusterInterval);},500);
          }
          else {
            clicked = true;
...
//This is the callback function which will be used to send the data over the sockets.
//We need to send the modified game array to the socket and redraw the screen.
function CheckIfMoveIsValid(modifiedGameArray,socket,ids){
  socket.emit("SwapAnimation",{ids : ids});
  //Give some time for swaping animation
  setTimeout(function(){
    socket.emit("Redraw",{array : modifiedGameArray});}
    ,130);//Give some time to animation then redraw the whole game.
}
